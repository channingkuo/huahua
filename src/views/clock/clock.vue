<template>
  <div class="clock">
    <a-tooltip placement="top" title="全屏">
      <a-icon class="size-change" type="fullscreen" @click="onFullScreen" />
    </a-tooltip>
    <div class="flex justify-center items-center w-full" :style="{ height: height + 'px' }">
...
export default {
  ...
  mounted() {
    window.onresize = () => {
      return (() => {
        ...
        this.height = getScreenHeight() - 47
      })()
    }
    this.$nextTick(() => {
      this.myChart = echarts.init(document.getElementById('clock'))
      this.loading = false
      setInterval(() => {
        let date = new Date()
        ...
        this.updateSeries(hour, this.option.series[0], 'hour')

        this.option.animationDurationUpdate = 300
        this.myChart.setOption(this.option, true)
        if (this.myChartFullScreen) {
          this.myChartFullScreen.setOption(this.option, true)
        }
        date = null
      }, 1000)
    })
  ...
  methods: {
    ...
    onFullScreen() {
      ...
      setTimeout(() => {
        this.myChartFullScreen = echarts.init(document.getElementById('clock-full-screen'))
